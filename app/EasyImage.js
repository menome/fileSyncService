 ...
 * 
 * This was embedded in here because we have a different use case and the stock error handling wasn't enough.
 */
var Q = require('q');
...
command('convert -version', function(err, stdout, stderr) {
	...
	if (err) {
		console.error(' ImageMagick Not Found'.red)
		console.error(' EasyImage requires ImageMagick to work. Install it from http://www.imagemagick.org/script/binary-releases.php.\n')
	}
...
function info(file) {
	...
	var parseSize = function(sizeString) {
		...
		if(!sizeArray || sizeArray.length < 3) return 0;
